<div
    x-data="{
        get currentStatus() { return $wire.status }
    }"
    @if($shouldPoll) wire:poll.2s="checkStatus" @endif
    x-on:trigger-download.window="
        const url = $event.detail.url || ($event.detail[0] || null);
        if (url) {
    ...
    "
    class="w-full max-w-4xl mx-auto relative min-h-[300px]"
>

    {{-- Error Banner --}}
    <div x-show="currentStatus === 'failed'" x-cloak
        x-transition:enter="transition ease-out duration-300"
        x-transition:enter-start="opacity-0 -translate-y-2"
        x-transition:enter-end="opacity-100 translate-y-0"
        class="mb-6 p-4 bg-red-500/10 border border-red-500/20 rounded-2xl backdrop-blur-xl">
        <div class="flex items-start gap-3">
            ...
            <div class="flex-1">
                ...
                <p class="text-red-300/70 text-xs sm:text-sm mt-1 leading-relaxed" x-text="$wire.errorMessage || 'An unknown error occurred.'"></p>
            </div>
    ...
    {{-- ============================================================ --}}
    {{-- State: URL Input Form (idle / failed) --}}
    {{-- ============================================================ --}}
    <div x-show="currentStatus === 'idle' || currentStatus === 'failed'" x-cloak
        x-transition:enter="transition ease-out duration-300"
            ...
            <div class="relative group">
                ...
                <div class="relative flex flex-col sm:flex-row items-stretch sm:items-center bg-slate-900/90 backdrop-blur-2xl rounded-2xl border border-white/10 p-2 gap-2">
                    ...
                    <button type="submit" id="download-submit-btn" wire:loading.attr="disabled"
                        ...
                        <span wire:loading.remove wire:target="submit">Analyze Link</span>
    ...
    {{-- ============================================================ --}}
    <div x-show="currentStatus === 'fetching_metadata'" x-cloak
        ...
        <div class="relative max-w-sm mx-auto">
            ...
            <div class="relative bg-slate-900/90 backdrop-blur-2xl rounded-3xl border border-white/10 p-8 sm:p-10 text-center">
                ...
                <div class="mt-6 h-1 bg-slate-800 rounded-full overflow-hidden">
                    <div class="h-full bg-gradient-to-r from-cyan-400 to-blue-500 rounded-full animate-progress-bar"></div>
                </div>
            </div>
        </div>
    </div>
    ...
    {{-- ============================================================ --}}
    <div x-show="currentStatus === 'metadata_fetched'" x-cloak
        ...
        class="w-full bg-slate-900/60 backdrop-blur-2xl rounded-3xl border border-white/10 p-6 sm:p-8">
        <div class="grid grid-cols-1 md:grid-cols-12 gap-8 items-start">
            
            {{-- Video Card Preview (5 cols) --}}
                            ...
                            @if (($f['type'] ?? '') === 'video')
                                ...
                                <div class="flex items-center justify-between p-3.5 bg-slate-800/40 hover:bg-slate-800/70 border border-white/5 rounded-xl transition duration-200">
                                    ...
                                    <button wire:click="startDownload('{{ $f['id'] }}', '{{ $f['quality'] }}')" 
                                        class="px-5 py-2 bg-blue-600 hover:bg-blue-500 text-white font-bold text-xs sm:text-sm rounded-lg shadow transition-all hover:scale-[1.03] active:scale-97 flex items-center gap-1.5">
                        ...
                        @foreach ($formats as $f)
                            @if (($f['type'] ?? '') === 'audio')
                                <div class="flex flex-col justify-between p-4 bg-slate-800/40 hover:bg-slate-800/70 border border-white/5 rounded-xl transition duration-200 gap-3">
                                    ...
                                    <button wire:click="startDownload('{{ $f['id'] }}', '{{ $f['quality'] }}')" 
                                        class="w-full py-2 bg-purple-600 hover:bg-purple-500 text-white font-bold text-xs rounded-lg shadow transition-all hover:scale-[1.02] active:scale-97 flex items-center justify-center gap-1.5">
    ...
    {{-- ============================================================ --}}
    <div x-show="currentStatus === 'processing'" x-cloak
        x-transition:enter="transition ease-out duration-400"
        x-transition:enter-start="opacity-0 scale-90"
        x-transition:enter-end="opacity-100 scale-100"
        x-transition:leave="transition ease-in duration-200"
        ...
        <div class="relative max-w-md mx-auto">
            ...
            <div class="relative bg-slate-900/90 backdrop-blur-2xl rounded-3xl border border-white/10 p-8 sm:p-10 text-center">
                ...
                <p class="text-slate-400 text-xs sm:text-sm leading-relaxed">
                    We are downloading your requested format and merging video & audio tracks using FFMPEG.
                </p>
    ...
    {{-- State: Completed --}}
    {{-- ============================================================ --}}
    <div x-show="currentStatus === 'completed'" x-cloak
        x-transition:enter="transition ease-out duration-500"
        x-transition:enter-start="opacity-0 scale-90"
        x-transition:enter-end="opacity-100 scale-100"
